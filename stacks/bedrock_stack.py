...
class BedrockStack(Stack):
    ...
    def __init__(
        ...
        id: str,
        dict1,
        search_lambda_arn=None,
        athena_lambda_arn=None,
        **kwargs,
    ) -> None:
        ...
        access_logs_bucket = s3.Bucket.from_bucket_name(
            self, "AccessLogsBucketName", Fn.import_value("AccessLogsBucketName")
        )
        ...
        schema_bucket = s3.Bucket(
            ...
            "schema-bucket",
            bucket_name=(
                "schema-bucket-" + str(hashlib.sha384(hash_base_string).hexdigest())[:15]
            ).lower(),
        ...
        NagSuppressions.add_resource_suppressions_by_path(
            ...
            [
                ...
                NagPackSuppression(
                    id="AwsSolutions-IAM5", reason="Policies are set by the Construct."
                ),
            ],
            True,
        )
        ...
        CfnOutput(
            self, "APISchemaBucket", value=schema_bucket.bucket_name, export_name="APISchemaBucket"
        )
        ...
        bedrock_agent_role = iam.Role(
            self,
            'bedrock-agent-role',
            role_name=f'AmazonBedrockExecutionRoleForAgents_'
        ...
        NagSuppressions.add_resource_suppressions_by_path(
            self,
            '/BedrockAgentStack/bedrock-agent-role/Resource',
            [
                NagPackSuppression(
                    id="AwsSolutions-IAM4",
                    reason="Permissive permissions required as per Bedrock agents documentation.",
                ),
            ],
            True,
        )
        ...
        invocation_logging_role = iam.Role(
            self,
            'invocation-logs-role',
            role_name=(
                "InvocationLogsRole-" + str(hashlib.sha384(hash_base_string).hexdigest())[:15]
            ).lower(),
            assumed_by=iam.ServicePrincipal('bedrock.amazonaws.com'),
        )
        ...
        NagSuppressions.add_resource_suppressions_by_path(
            ...
            [
                NagPackSuppression(
                    id="AwsSolutions-IAM4", reason="Policies are set by Custom Resource."
                ),
                NagPackSuppression(
                    id="AwsSolutions-IAM5", reason="Policies are set by Custom Resource."
                ),
            ],
            True,
        )
        ...
        NagSuppressions.add_resource_suppressions_by_path(
            ...
            [
                NagPackSuppression(
                    id="AwsSolutions-IAM4",
                    reason="Contains a resouce wildecard for bedrock as this is a global setting.",
                ),
                NagPackSuppression(
                    id="AwsSolutions-IAM5", reason="Policies are set by Custom Resource."
                ),
        ...
        cfn_guardrail = bedrock.CfnGuardrail(
            self,
            "CfnGuardrail",
            ...
            content_policy_config=bedrock.CfnGuardrail.ContentPolicyConfigProperty(
                filters_config=[
                    bedrock.CfnGuardrail.ContentFilterConfigProperty(
                        input_strength="NONE", output_strength="NONE", type="PROMPT_ATTACK"
                    ),
                    bedrock.CfnGuardrail.ContentFilterConfigProperty(
        ...
        update_agent_cr = cr.AwsCustomResource(
            self,
            "UpdateAgentCustomResource",
            on_create=cr.AwsSdkCall(
        ...
        # Define IAM permission policy for the custom resource
        update_agent_cr.grant_principal.add_to_principal_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=["bedrock:UpdateAgent", "iam:CreateServiceLinkedRole", "iam:PassRole"],
                resources=[
