...
class DataFoundationStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        logs_bucket = s3.Bucket(
            ...
            "LogsBucket",
            bucket_name=(
                "logs-bucket-" + str(hashlib.sha384(hash_base_string).hexdigest())[:15]
            ).lower(),
        ...
        NagSuppressions.add_resource_suppressions_by_path(
            self,
            '/DataStack/LogsBucket/Resource',
            [
                NagPackSuppression(
                    id="AwsSolutions-S1",
                    reason="TMP to speed up deployment",
                )
            ],
            True,
        )
        ...
        kb_bucket = s3.Bucket(
            ...
            "Knowledgebase",
            bucket_name=(
                "knowledgebase-bucket-" + str(hashlib.sha384(hash_base_string).hexdigest())[:15]
            ).lower(),
            auto_delete_objects=True,
            versioned=True,
            removal_policy=RemovalPolicy.DESTROY,
        ...
        upload_docs = s3d.BucketDeployment(
            self,
            "KnowledgebaseDocs",
            sources=[s3d.Source.asset("assets/kb-docs/")],
            destination_bucket=kb_bucket,
            destination_key_prefix="fao-basic-texts/",
